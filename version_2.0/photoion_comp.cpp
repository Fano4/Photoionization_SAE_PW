...
int photoion_comp(int argc, char* argv[])
{
   ...
   double Pi=acos(-1);
    ...
    int nky(64);
    int nkz(64);
    double xmin(-37.5);//-27.401029);
    double xmax(-37.5+nx*0.5);//27.842971);
    double ymin(-37.5);//-27.401029);
    double ymax(-37.5+ny*0.5);//27.842971);
    double zmin(-37.5);//-27.010679);
    double zmax(-37.5+nz*0.5);//28.233321);
    double x;
    double y;
    double z;
...
//*****************************GET DATA FOR COMPUTING DYSON ORBITALS FROM MOLPRO OUTPUT FILE*****************************
   ...
   transition_dipole[2]=new double[n_occ*n_occ];
    ...
    for(int i=0;i!=n_sym;i++)
    {
       ...
       std::cout<<"CI size neut sym "<<i<<" : "<<ci_size_neut_sym[i]<<std::endl;
       ci_size_cat+=ci_size_cat_sym[i];
       std::cout<<"CI size cat sym "<<i<<" : "<<ci_size_cat_sym[i]<<std::endl;
    }
...
/*      for(int i=0;i!=n_states_neut;i++)
      {
         for(int j=0;j!=n_occ;j++)
         {
            dyson_mo_basis_coeff[i*n_states_cat*n_occ+j]=0;
         }
         dyson_mo_basis_coeff[i*n_states_cat*n_occ+i]=1;
      }*/
//    DEBOGAGE!!!
